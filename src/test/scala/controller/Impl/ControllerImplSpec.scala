...
class ControllerImplSpec extends AnyWordSpec with Matchers {
  ...
  object TestHelper {
    ...
    def createGameWithTable(idx: Int = 0): Game = {
      ...
      Game(List(p1, p2), idx, List(testCard), Some(testCard), ActionState.ChooseCard, TurnState.PlayerTurn(p1))
    }
  }
  ...
  "ControllerImpl" should {
    ...
    "test undo with command in stack" in {
      ...
      val cmd = new Command {
        override def execute(game: Game): (Game, Integer) = (game.copy(index = 1), 0)
        override def undo(currentGame: Game, previousGame: Game): Game = previousGame
      }
      
      val (newManager, newGame, _) = controller.cmdManager.executeCommand(cmd, game)
    ...
    "test saveGame with failure" in {
      ...
      val cmdManager = new CommandManager()
      val fileIO: FileIOInterface = new FileIOInterface {
        override def save(game: Game): Try[Unit] = Failure(new Exception("Save failed"))
        override def load(): Try[Game] = Success(game)
      }
    ...
    "test loadGame method" in {
      val game = Game(Nil, 0, Nil, None, ActionState.None, TurnState.None)
      val loadedGame = Game(TestHelper.player("Loaded", Nil, 0) :: Nil, 1, Nil, None, ActionState.None, TurnState.None)
      val cmdManager = new CommandManager()
      val fileIO: FileIOInterface = new FileIOInterface {
        override def save(game: Game): Try[Unit] = Success(())
    ...
    "test gameloop with Hint=1 (ChooseColour)" in {
      ...
      val customGameStates = new GameStatesImpl(
        ...
        (game)
          ...
          def nextPlayerIndex(currentIndex: Int, playerCount: Int, skipNext: Boolean): Int = currentIndex
          def parseCardIndex(index: Int, player: Player, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def playCard(game: Game, playerIdx: Int, cardIdx: Int): (Game, Integer) = (game, 1)
          def playCardIfValid(card: Card, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def plusN(game: Game, nextPlayerIndex: Int, card: Card, n: Int): Game = game
          def start(p1: Player, p2: Player, gameStates: GameStates): Game = Game(List(p1, p2), 0, Nil, None, ActionState.None, TurnState.None)
          def turn(card: Card, game: Game, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
      ...
      val controller = new ControllerImpl(game, cmdManager, customGameStates, fileIO)
    ...
    "test gameloop with Hint=3 (DrawCard)" in {
      ...
      val customGameStates = new GameStatesImpl(
        ...
        (game)
          ...
          def nextPlayerIndex(currentIndex: Int, playerCount: Int, skipNext: Boolean): Int = currentIndex
          def parseCardIndex(index: Int, player: Player, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def playCard(game: Game, playerIdx: Int, cardIdx: Int): (Game, Integer) = (game, 3)
          def playCardIfValid(card: Card, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def plusN(game: Game, nextPlayerIndex: Int, card: Card, n: Int): Game = game
          def start(p1: Player, p2: Player, gameStates: GameStates): Game = Game(List(p1, p2), 0, Nil, None, ActionState.None, TurnState.None)
          def turn(card: Card, game: Game, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
    ...
    "test gameloop with Hint=6 (NextPlayer)" in {
      ...
      val customGameStates = new GameStatesImpl(
        ...
        (game)
          ...
          def nextPlayerIndex(currentIndex: Int, playerCount: Int, skipNext: Boolean): Int = currentIndex
          def parseCardIndex(index: Int, player: Player, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def playCard(game: Game, playerIdx: Int, cardIdx: Int): (Game, Integer) = (game, 6)
          def playCardIfValid(card: Card, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def plusN(game: Game, nextPlayerIndex: Int, card: Card, n: Int): Game = game
          def start(p1: Player, p2: Player, gameStates: GameStates): Game = Game(List(p1, p2), 0, Nil, None, ActionState.None, TurnState.None)
          def turn(card: Card, game: Game, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
    ...
    "test gameloop with default branch (else)" in {
      ...
      val customGameStates = new GameStatesImpl(
        ...
        (game)
          ...
          def nextPlayerIndex(currentIndex: Int, playerCount: Int, skipNext: Boolean): Int = currentIndex
          def parseCardIndex(index: Int, player: Player, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def playCard(game: Game, playerIdx: Int, cardIdx: Int): (Game, Integer) = (game, 2)
          def playCardIfValid(card: Card, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
          def plusN(game: Game, nextPlayerIndex: Int, card: Card, n: Int): Game = game
          def start(p1: Player, p2: Player, gameStates: GameStates): Game = Game(List(p1, p2), 0, Nil, None, ActionState.None, TurnState.None)
          def turn(card: Card, game: Game, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
    ...
    "test all parameters and branches with various hint values" in {
      ...
      val varyingGameStates = new GameStatesImpl(
        ...
        (game)
          ...
          def playCard(game: Game, playerIdx: Int, cardIdx: Int): (Game, Integer) = {
            val hint = if (hintIndex < hintSequence.length) hintSequence(hintIndex) else 0
            hintIndex += 1
            (game, hint)
          }
          def playCardIfValid(card: Card, game: Game, tableCard: Card, currentPlayerIndex: Int): (Game, Integer) = (game, 0)
      ...
      val controller = new ControllerImpl(game, cmdManager, varyingGameStates, fileIO)
    ...
    "test different game states with varying index" in {
      ...
      val controller1 = new ControllerImpl(game1, cmdManager, TestHelper.StubGameStates, fileIO)
      val controller2 = new ControllerImpl(game2, cmdManager, TestHelper.StubGameStates, fileIO)
