...
class GameSpec extends AnyWordSpec with Matchers {
  ...
  object TestHelper {
    ...
    def player(name: String, hand: List[Card] = List(), index: Int) =
      Player(name, hand, index)
  ...
  "A Game" should {
    ...
    "correctly convert to a GameBuilder with the same state" in {
      val player1 = TestHelper.player(
        "Alice",
        List(TestHelper.card(Coulor.red, Symbol.Three)),
        0
      )
      val player2 = TestHelper.player(
        "Bob",
        List(TestHelper.card(Coulor.blue, Symbol.Reverse)),
        1
      )
      val players = List(player1, player2)

      val deck = List(
        TestHelper.card(Coulor.green, Symbol.Seven),
        TestHelper.card(Coulor.yellow, Symbol.Reverse)
      )
