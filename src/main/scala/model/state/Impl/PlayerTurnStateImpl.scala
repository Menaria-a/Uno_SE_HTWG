...
case object PlayCardStateImpl extends PlayCardState:
  ...
  def turn(
    ...
    val updatedPlayers = game.player.updated(
      currentPlayerIndex,
      currentPlayer.copy(hand = updatedHand)
    )
    ...
    card.symbol match
      case Symbol.Plus_2 =>
        val nextIndex =
          nextPlayerIndex(currentPlayerIndex, game.player.size, true)
      ...
      case Symbol.Wish =>
        val nextIndex =
          nextPlayerIndex(currentPlayerIndex, game.player.size, false)
