...
class FileIOXml @Inject() (Path: String, gameStates: GameStates) extends FileIOInterface:
    ...
    private def cardToXml(card: Card): Elem =
        <Card>
            <Colour>{ card.colour.toString }</Colour>
    ...
    private def xmlToGame(node: Node): Game =
        val player = (node \ "Players" \ "Player").map(xmlToPlayer).toList
        val index = (node \ "Index").text.toInt
        val deck = (node \ "Deck" \ "Card").map(xmlToCard).toList
        val table  = (node \ "Table" \ "Card").headOption.map(xmlToCard)
        val actionState =
            (node \ "ActionState").text match
        ...
        val turnState =
            (node \ "TurnState").headOption.map(xmlToTurnState).getOrElse(TurnState.None)
    
        Game(player, index, deck, table, actionState, turnState)

    private def xmlToPlayer(node: scala.xml.Node): Player =
        Player(
        (node \ "Name").text,
    ...
    private def xmlToTurnState(node: Node): TurnState =
        node.label match
            case "PlayerTurn" => TurnState.PlayerTurn(xmlToPlayer((node \ "Player").head))
            case "GameWon"    => TurnState.GameWon(xmlToPlayer((node \ "Player").head))
