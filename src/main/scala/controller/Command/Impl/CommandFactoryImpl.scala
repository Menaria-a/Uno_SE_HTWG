...
class CommandFactoryImpl @Inject(gameStates : GameStates) extends CommandFactory:

    override def playCard(playerIdx: Int, cardIdx: Int, input: Int): PlayCardCommandT =
    ...
    override def chooseColour(hand: Card, input: PlayerInput): ChooseColourCommandT =
